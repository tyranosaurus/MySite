...
<%-- 				<c:set var="count" value="${fn:length(list) }"/> --%>
					...
					<tr>
						...
						<c:choose>
							<c:when test="${boardVo.orderNo == '1' }">							
								<td style="text-align:left"><a href="/mysite/board?a=view&no=${boardVo.no }">${boardVo.title }</a></td>
							</c:when>
							
							<c:otherwise> 
								<td style="text-align:left; padding-left:${20*boardVo.depth}px"><img src="${pageContext.request.contextPath}/assets/images/reply.png"><a href="/mysite/board?a=view&no=${boardVo.no }">${boardVo.title }</a></td>
							</c:otherwise>
						</c:choose>
				...
				<%-- 	<c:out value="${authUser.no}"></c:out>
						...
						<c:if test="${authUser.no == boardVo.userNo }">
							...
							<a href="/mysite/board?a=delete&no=${boardVo.no }" class="del">삭제</a>
				...
				<div class="pager"> 
 				...
 				<c:choose>
 					<c:when test="${map.firstPage > 1 }">
 					    <li><a href="/mysite/board?a=list&pageGroup=${map.pageGroup - 1 }&firstPage=${(map.pageGroup-1)*3 - 2 }&pageNum=${(map.pageGroup-1)*3 - 2 }&kwd=${map.kwd}">◀</a></li>
 					</c:when>
 				</c:choose>


 						<c:forEach begin="${map.firstPage }" end="${map.lastPage }" var="i" step="1">
 							 <li><a href="/mysite/board?a=list&kwd=${map.kwd }&pageGroup=${map.pageGroup }&pageNum=${i }&firstPage=${map.firstPage }&kwd=${map.kwd}">${i } </a></li> 
 						</c:forEach>
 						
 						
 				<c:choose>
 					 <c:when test="${map.lastPage < map.totalPage }">
 					    <li><a href="/mysite/board?a=list&pageGroup=${map.pageGroup + 1 }&firstPage=${map.pageGroup*3 + 1 }&pageNum=${map.pageGroup*3 + 1 }&kwd=${map.kwd}">▶</a></li> 
 					 </c:when>
 				</c:choose>
